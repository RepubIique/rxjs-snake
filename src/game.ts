...
class Board {
    ...
    startSnake(size: number, heading: Heading): CellLocation[] {
        const centre = new CellLocation(Math.floor(this.width / 2), Math.floor(this.height / 2));
    ...
    getRect(cellLocation: CellLocation): { x: number, y: number, width: number, height: number } {
        return {
    ...
    random(): CellLocation {
        return new CellLocation(Math.floor(Math.random() * this.width), Math.floor(Math.random() * this.height));
    }
}
...
class Game {
    ...
    gameOn() {
        ...
        const heading$: Observable<Heading> = keyDowns$.pipe(
            filter((curr: KeyboardEvent) => ['ArrowUp', 'ArrowRight', 'ArrowDown', 'ArrowLeft'].indexOf(curr.key) !== -1),
            ...
            .pipe(
                ...
                scan((acc: GameState, curr: [number, Heading]) => {
                    ...
                    return {
                        isAlive: true,
                        snake: {
                            body: didEat ? [...acc.snake.body, newHead] : [...acc.snake.body.slice(1), newHead],
                            heading: newHeading,
                        },
                        snakeEvolution: {
                            newHead: [newHead],
                        ...
                        foodLocation: didEat || !acc.foodLocation ? this.board.random() : acc.foodLocation,
                    };
                }, state0),
                startWith(state0),
